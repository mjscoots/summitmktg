...
export function OnboardingAlert() {
  ...
  return (
    <Card className={`mb-4 border ${isWarn ? 'border-primary/40 bg-primary/5' : 'border-primary/40 bg-primary/5'}`}>
      <div className="p-4 flex items-start gap-3">
        <div className={`rounded-full p-1.5 ${isWarn ? 'bg-primary/15' : 'bg-primary/15'}`}>
          <AlertTriangle className={`w-4 h-4 ${isWarn ? 'text-primary' : 'text-primary'}`} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-sm font-semibold text-foreground">Join Onboarding Zoom</h3>
            <Badge variant="outline" className={`text-[9px] ${isWarn ? 'text-primary border-primary/30' : 'text-primary border-primary/30'}`}>
              {config.label}
            </Badge>
          </div>
          ...
          <div className="mt-3">
            <Button
              size="sm"
              variant="outline"
              className={`gap-1.5 text-xs h-7 ${isWarn ? 'border-primary/30 text-primary/80 hover:bg-primary/10' : 'border-primary/30 text-primary hover:bg-primary/10'}`}
