CREATE OR REPLACE FUNCTION public.get_my_money_summary(_target uuid DEFAULT NULL::uuid)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT sum(r.revenue) INTO _imported_revenue FROM public.rep_revenue r WHERE r.user_id = _t;

  _pest_verticals := jsonb_build_object(
    'pay_scale', _com.pay_scale,
    'signs', _com.signs,
    'avg_account_value', _com.avg_account_value,
    'active_revenue', COALESCE(_imported_revenue, _com.active_revenue),
    'imported_revenue', _imported_revenue,
  ...
  _fiber := jsonb_build_object(
    ...
    'pay_gross', (SELECT sum(gross) FROM public.fiber_pay_weeks WHERE user_id=_t),
    'pay_overrides', (SELECT sum(overrides) FROM public.fiber_pay_weeks WHERE user_id=_t),
    'pay_costs', (SELECT sum(costs) FROM public.fiber_pay_weeks WHERE user_id=_t)
  );
  ...
  RETURN jsonb_build_object(
    ...
    'months', _months,
    'events', _events,
    'sources', public.get_money_sources()
  );
END;
$function$;