-- DM message policies are evaluated as the signed-in caller, so the helpers they
-- call must be executable by that role. Verification showed message inserts in a
-- DM failing with "permission denied for function is_dm_channel".
GRANT EXECUTE ON FUNCTION public.is_dm_channel(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_dm_member(text, uuid) TO authenticated;
REVOKE ALL ON FUNCTION public.is_dm_channel(text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.is_dm_member(text, uuid) FROM PUBLIC, anon;

-- search_people reported view_level 'none' even when a number was returned.
-- Return the subject's own phone visibility setting instead.
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'team_name', t.name,
      'phone', CASE WHEN public.can_see_phone(_uid, p.user_id) THEN p.phone ELSE NULL END,
