...
BEGIN
  ...
  _can_see_up := _visibility <> 'self'
    AND (_is_staff OR _rookies OR COALESCE(_rank.name,'') NOT LIKE 'Tier 1%');
    ...
    IF _enr.id IS NOT NULL AND _v.vertical = 'Fiber' THEN
      ...
      IF _carrier.id IS NOT NULL AND _rank.id IS NOT NULL THEN
        ...
        IF _stack.id IS NULL THEN
          _card := _card || jsonb_build_object('stack_note','Rate not set','carrier',_carrier.name);
        ...
        ELSE
          _card := _card || jsonb_build_object(
            ...
            'stack_note', CASE WHEN _stack.value IS NULL THEN 'Rate not set' ELSE NULL END,
          ...
          IF _can_see_up THEN
            ...
            IF _visibility = 'full_chain' THEN
              SELECT jsonb_agg(jsonb_build_object('rank', COALESCE(s.label, r.name), 'stack', s.value)
               ...
               WHERE s.vertical='Fiber' AND s.carrier_id=_carrier.id
                 AND COALESCE(s.sort_order, r.sort_order) > COALESCE(_rank.sort_order,0)
      ...
      ELSIF _rank.id IS NULL THEN
        _card := _card || jsonb_build_object('stack_note','Rate not set');
      END IF;
          ...
          WHEN q.rule_type = 'team_leads_under' THEN (
            SELECT COUNT(*)::int FROM public.rep_vertical_enrollments e
             ...
             WHERE e.paired_manager = _uid AND e.vertical = _v.vertical
               AND rr.name LIKE 'Team Lead%')
          WHEN q.rule_type = 'managers_under' THEN (
            SELECT COUNT(*)::int FROM public.rep_vertical_enrollments e
              JOIN public.profiles pr ON pr.user_id = e.user_id
              JOIN public.ranks rr ON rr.id = pr.rank_id
             WHERE e.paired_manager = _uid AND e.vertical = _v.vertical
               AND (rr.name LIKE 'Manager%' OR rr.name LIKE 'Org stack%'))
          ELSE NULL END
  ...
  _tier := public.my_fiber_tier(_uid);

  RETURN jsonb_build_object(
    'rank', _rank.name,
    'rank_is_summit', _rank.id IS NULL AND _role IN ('owner','admin'),
    'next_rank', _next.name,
    'rank_label', _tier->>'rank_label',
    'next_tier_label', _tier->>'next_tier_label',
    'next_tier_gap', (_tier->'next_tier_gap'),
    'next_tier_progress', (_tier->'next_tier_progress'),
    'fiber_per_install', (_tier->'per_install'),
