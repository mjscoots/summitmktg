...
REVOKE ALL ON FUNCTION public.manager_owed(uuid) FROM anon;
...
BEGIN
  ...
  FOR _r IN
    SELECT DISTINCT ur.user_id, COALESCE(p.full_name,'Manager') AS full_name
    FROM public.user_roles ur
    JOIN public.profiles p ON p.user_id = ur.user_id
    WHERE ur.role IN ('manager','president')
...
REVOKE ALL ON FUNCTION public.owed_by_manager() FROM anon;
