  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT count(*)::int AS application_count,
        ...
        jsonb_agg(jsonb_build_object('id', a.id, 'vertical', a.vertical, 'status', a.status, 'created_at', a.created_at, 'updated_at', a.updated_at) ORDER BY a.created_at DESC) AS applications
      FROM public.vertical_applications a
      WHERE a.user_id = p.user_id
    ) va ON true
    LEFT JOIN LATERAL (
      SELECT count(*)::int AS personal_link_count,
        jsonb_agg(jsonb_build_object('id', l.id, 'title', l.title, 'url', l.url, 'created_at', l.created_at, 'is_active', l.is_active) ORDER BY l.created_at DESC) AS personal_links
      FROM public.managed_links l
      WHERE l.created_by = p.user_id AND l.link_scope = 'personal' AND l.is_active = true
    ) ml ON true
    WHERE p.user_id IS NOT NULL
      AND COALESCE(p.status::text, 'active') <> 'rejected'
  ) rows;
  RETURN result;
END;
$$;
REVOKE ALL ON FUNCTION public.rep_progress_summary() FROM PUBLIC, anon, service_role;
