...
CREATE OR REPLACE FUNCTION public.get_all_time_leaderboard(_limit integer DEFAULT 50)
 RETURNS TABLE(
   ...
   total_points integer, current_streak integer, legacy_points integer,
   new_hours_points integer, new_event_points integer,
   ...
   reaction_points integer, one_on_one_points integer, threshold_bonus integer,
   total_time_minutes integer, lessons_completed bigint, videos_watched bigint
 )
 LANGUAGE plpgsql
